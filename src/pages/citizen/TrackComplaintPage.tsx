      ...
      <div className="max-w-7xl mx-auto px-4 py-8 sm:py-12">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-800">Track My Complaint</h1>
              <p className="text-sm text-slate-500">View the current status and progress of your complaint</p>
            </div>
          </div>
        </div>

        {/* ── Lookup form (Compact & Horizontal on Desktop) ── */}
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-6 mb-8 max-w-4xl">
          <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4 items-end">
            <div className="w-full md:flex-1">
              <label htmlFor="complaint-id" className="block text-sm font-semibold text-slate-700 mb-1.5">
                Complaint ID
              </label>
              <input
                id="complaint-id"
                ...
                className="w-full rounded-xl border border-slate-200 px-4 py-2.5 font-mono text-sm tracking-wider focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary placeholder:text-slate-300 transition-shadow"
              />
            </div>

            <div className="w-full md:flex-1">
              <label htmlFor="track-aadhaar" className="block text-sm font-semibold text-slate-700 mb-1.5">
                Aadhaar Number
              </label>
              <input
                id="track-aadhaar"
                ...
                className="w-full rounded-xl border border-slate-200 px-4 py-2.5 font-mono tracking-widest text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary placeholder:text-slate-300 transition-shadow"
                maxLength={14}
              />
            </div>
            ...
            <div className="w-full md:w-auto">
              <Button type="submit" size="lg" loading={loading} className="w-full md:w-auto py-2.5 px-6">
                {loading ? 'Searching...' : 'Search Complaint'}
              </Button>
            </div>
          </form>

          {error && (
            <div className="flex items-start gap-2.5 p-3.5 mt-4 bg-red-50 border border-red-200 text-red-700 text-sm rounded-xl animate-fade-in">
        ...
        {loading && !result && (
          <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
            ...
            <p className="text-slate-500 font-medium">Searching for your complaint...</p>
          </div>
        )}
        ...
        {result && (
          <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 animate-fade-in">
            ...
            <div className="lg:w-3/4 space-y-6">
              {result.isDemo && (
                <div className="flex items-center gap-2 text-xs bg-amber-50 border border-amber-200 text-amber-700 rounded-xl px-4 py-2.5">
                  <span>🎯</span>
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col md:flex-row">
                <div className="p-6 md:p-8 flex-1">
                  <h2 className="text-lg font-bold text-slate-800 mb-6">Complaint Details</h2>
                  
                  <div className="grid grid-cols-2 gap-y-6 gap-x-8 mb-6">
                    <div>
                      <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Complaint ID</p>
                      <p className="text-sm font-mono font-bold text-slate-800">{displayCode(result.complaint)}</p>
                    </div>
                    <div>
                      <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Submitted On</p>
                      <p className="text-sm font-medium text-slate-700">{formatDate(result.complaint.created_at)}</p>
                    </div>
                    <div>
                      <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Category</p>
                      <p className="text-sm font-medium text-slate-700">{result.complaint.category}</p>
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-700 leading-relaxed bg-slate-50 border border-slate-100 rounded-xl px-4 py-3">
                ...
                {result.complaint.photo_url && (
                  <div className="md:w-64 border-t md:border-t-0 md:border-l border-slate-100 bg-slate-50 p-4 flex flex-col items-center justify-center shrink-0">
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-3 w-full text-left">Attached Photo</p>
                    <div className="rounded-xl overflow-hidden shadow-sm border border-slate-200 w-full h-40 relative">
              ...
              {(() => {
                ...
                return (
                  <div className="bg-white rounded-2xl border border-success-200 shadow-sm overflow-hidden flex flex-col md:flex-row">
                    <div className="p-6 md:p-8 flex-1 bg-success-50/50">
                      <div className="flex items-center gap-2 mb-4">
                        <span className="text-xl">✅</span>
                        <h2 className="text-lg font-bold text-success-800">Resolution Details</h2>
                      </div>
                      <p className="text-sm text-success-700 leading-relaxed font-medium mb-4">
                        Your complaint has been successfully resolved by the municipal authority. 
                        Please see the attached completion proof photo.
                      </p>
                      {resolvedHistory && (
                        <div className="bg-white/60 border border-success-200/60 rounded-xl p-4 mt-4">
                          <div className="flex flex-col sm:flex-row gap-4 sm:gap-8">
                            <div>
                              <p className="text-[10px] font-bold text-success-600/70 uppercase tracking-wider mb-1">Resolved On</p>
                              <p className="text-sm font-semibold text-success-800">{formatDate(resolvedHistory.changed_at)}</p>
                            </div>
                            {resolvedHistory.note && (
                              <div className="flex-1">
                                <p className="text-[10px] font-bold text-success-600/70 uppercase tracking-wider mb-1">Authority Note</p>
                                <p className="text-sm font-medium text-success-800">{resolvedHistory.note}</p>
                              </div>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="md:w-64 border-t md:border-t-0 md:border-l border-success-100 bg-white p-4 flex flex-col items-center justify-center shrink-0">
                      <p className="text-[11px] font-bold text-success-600 uppercase tracking-wider mb-3 w-full text-left">Completion Proof</p>
                      <div className="rounded-xl overflow-hidden shadow-sm border border-success-200 w-full h-40 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                  <div className="px-6 py-5 border-b border-slate-100">
                    <h2 className="text-base font-bold text-slate-800">Complaint Location</h2>
                  </div>
                  <div className="p-6 space-y-4 flex-1 flex flex-col">
                    <div>
                      <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Address</p>
                      <div className="flex items-start gap-1.5">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 text-slate-400 shrink-0 mt-0.5">
                        ...
                        <p className="text-sm text-slate-700">{result.complaint.address || 'Location provided via map coordinates.'}</p>
                      </div>
                    </div>
                    {result.complaint.lat && result.complaint.lng && (
                      <div className="rounded-xl overflow-hidden border border-slate-200 mt-auto flex-1 min-h-[200px] flex flex-col">
                        ...
                        <div className="bg-slate-50 px-4 py-2 text-xs font-mono text-slate-500 border-t border-slate-200 shrink-0">
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                  <div className="px-6 py-5 border-b border-slate-100">
                    <h2 className="text-base font-bold text-slate-800">Status Timeline</h2>
                  </div>
                  <div className="p-6 flex-1 flex flex-col justify-center">
                    {isRejected ? (
                      <RejectedTimeline history={result.history} createdAt={result.complaint.created_at} />
                    ) : (
                      <PipelineTimeline currentStatus={result.complaint.status} history={result.history} createdAt={result.complaint.created_at} />
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="lg:w-1/4 space-y-6">
              ...
              <StatusWidget status={result.complaint.status} isRejected={isRejected} />
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                <div className="p-2">
                  <button className="w-full flex flex-col text-left px-4 py-3 hover:bg-slate-50 transition-colors rounded-xl group">
                    <span className="text-sm font-semibold text-slate-800 group-hover:text-primary transition-colors">Having issues?</span>
                    <span className="text-xs text-slate-500 mt-0.5">Contact our support team.</span>
                  </button>
                  <button className="w-full flex flex-col text-left px-4 py-3 hover:bg-slate-50 transition-colors rounded-xl group mt-1">
                    <span className="text-sm font-semibold text-slate-800 group-hover:text-primary transition-colors">General Information</span>
                    <span className="text-xs text-slate-500 mt-0.5">Learn more about the process.</span>
                  </button>
                </div>
              </div>
            </div>
...
function PipelineTimeline({ currentStatus, history, createdAt }: { currentStatus: ComplaintStatus; history: StatusHistoryRow[]; createdAt: string }) {
          ...
          <li key={stage.key} className="flex gap-4">
            ...
            <div className="flex flex-col items-center">
              <div className={[
                ...
                state === 'current' ? 'bg-white border-primary ring-4 ring-primary/20' :
              ...
              {!isLast && (
                <div className={`w-0.5 flex-1 my-1 ${state === 'done' ? 'bg-primary' : 'bg-slate-200'}`} />
              )}
            </div>

            {/* Label + content */}
            <div className={`pb-6 flex-1 ${isLast ? 'pb-0' : ''}`}>
              <p className={`text-sm font-semibold ${state === 'upcoming' ? 'text-slate-400' : 'text-slate-800'}`}>
                {stage.label}
              </p>
              {state !== 'upcoming' && timestamp && (
                <p className="text-xs text-slate-400 mt-0.5">{formatDateTime(timestamp)}</p>
              )}
              {state === 'upcoming' && (
...
function StatusWidget({ status, isRejected }: { status: ComplaintStatus, isRejected: boolean }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden text-center p-6 sm:p-8">
      <div className={`w-16 h-16 ${bgClass} rounded-2xl flex items-center justify-center mx-auto mb-4`}>
        {icon}
      </div>
...
function RejectedTimeline({ history, createdAt }: { history: StatusHistoryRow[]; createdAt: string }) {
  const allItems = [
        ...
        return (
          <li key={idx} className="flex gap-4">
            <div className="flex flex-col items-center">
              ...
              {!isLast && <div className="w-0.5 flex-1 my-1 bg-slate-200" />}
            </div>
            <div className={`flex-1 ${isLast ? '' : 'pb-6'}`}>
              <p className="text-sm font-semibold text-slate-800">
                {STATUS_LABELS[item.status] || item.status}
              </p>
              <p className="text-xs text-slate-400 mt-0.5">{formatDateTime(item.changed_at)}</p>
              {item.note && (
                <p className="text-xs text-slate-600 mt-1.5 bg-red-50 border border-red-100 rounded-lg px-3 py-2">
                  {item.note}
                </p>
              )}
            </div>
